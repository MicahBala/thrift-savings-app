...
const DashboardLayout = () => {
  ...
  // Fetch the user data on mount to get their initials
  useEffect(() => {
    const userString = localStorage.getItem('thrift_user');
    if (userString) {
      const user = JSON.parse(userString);
      if (user.fullName) {
        // Grab the first letter of their full name
        setInitials(user.fullName.charAt(0).toUpperCase());
      }
    }
  }, []);

  const handleLogout = () => {
    // 1. Clear the user token and data from storage
    ...
    <div className="min-h-screen bg-[#F8F9FA] flex flex-col font-inter">
      ...
      <header className="bg-white border-b border-[#E1E3E4] h-20 flex items-center justify-between px-6 lg:px-10 sticky top-0 z-40">
        <div className="flex items-center gap-12">
          <Link
            to="/dashboard"
            className="text-2xl font-bold font-manrope text-[#0037B0]"
          >
            Thrift Sync
          </Link>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="flex flex-col w-20 lg:w-64 bg-white border-r border-[#E1E3E4] shrink-0 p-4 lg:p-6 space-y-2 relative z-30 transition-all duration-300">
          <div className="text-xs font-bold text-[#747686] uppercase tracking-wider mb-4 ml-3 hidden lg:block">
            Menu
          </div>
